...
static amrex::Geometry DefineGeometry( //
    int base_n, int level, amrex::Real domain_length);
...
DefineIOProcessorDM(const amrex::BoxArray &ba);
static void DefineFABs( //
    amrex::MultiFab &pressure, std::array<amrex::MultiFab, 3> &velocity,
...
static void RecursiveCompositeSolve( //
    ...
    int max_iterations, int base_n, int nLevels, int level,
    PressureSolve::StencilType stencil_type);
static amrex::Real ExpectedPressure( //
    const amrex::Geometry &geom, int i, int j, int k, int base_n, int nLevels);
static void GaussSeidelIteration( //
    bool use_expected_BCs, amrex::MultiFab &pressure,
    const amrex::MultiFab &divergence, const amrex::Geometry &geom,
    int iteration, int base_n, int nLevels,
    const PressureSolve::StencilBase &stencil);
static void FillPressureGhostCells( //
    LevelData &fine_level, const LevelData &crse_level);
static amrex::Real ComputeResidual( //
    const amrex::MultiFab &pressure, const amrex::MultiFab &divergence,
    const amrex::Geometry &geom, const PressureSolve::StencilBase &stencil);
static void ComputeDivergence( //
    amrex::MultiFab &divergence, const std::array<amrex::MultiFab, 3> &velocity,
    const amrex::Geometry &geom);
static void SolvePressureIterations( //
    bool use_expected_BCs, amrex::MultiFab &pressure,
    const amrex::MultiFab &divergence, const amrex::Geometry &geom,
    amrex::Real tolerance, int max_iterations, int base_n, int nLevels,
    const PressureSolve::StencilBase &stencil);
static amrex::MultiFab ComputeFluxCorrection( //
    const amrex::MultiFab &crse_pressure, const amrex::MultiFab &fine_pressure,
    const amrex::Geometry &crse_geom, const amrex::Geometry &fine_geom,
    PressureSolve::StencilType stencil_type);
static void SolvePressureCorrection( //
    amrex::MultiFab &crse_pressure, const amrex::MultiFab &fine_pressure,
    const amrex::Geometry &crse_geom, const amrex::Geometry &fine_geom,
    amrex::Real tolerance, int max_iterations, int base_n, int nlevels,
    PressureSolve::StencilType stencil_type);
static int CalculateNForLevel(int base_n, int level);
static amrex::Real Phi( //
    amrex::Real r_x, amrex::Real r_y, amrex::Real r_z, amrex::Real ri_x,
    amrex::Real ri_y, amrex::Real ri_z, amrex::Real dx);
static void
PrintCorrectionStatistics(const amrex::MultiFab &correction_solution);
...
void CompositeSolve( //
  ...
  RecursiveCompositeSolve( //
      composite_levels, tolerance, max_iterations, base_n, nLevels, 0,
      stencil_type);
  for (int lev = nLevels - 2; lev >= 0; --lev) {
    amrex::average_down( //
        composite_levels[lev + 1].pressure, composite_levels[lev].pressure, 0,
        1, 2);
  }
}
...
void RecursiveCompositeSolve( //
  ...
  if (level > 0) {
    ...
    FillPressureGhostCells( //
        composite_levels[level], composite_levels[level - 1]);
  }

  // Single-level solve on N
  SingleLevelPressureSolve( //
      composite_levels[level].pressure, composite_levels[level].velocity,
      composite_levels[level].geom, tolerance, max_iterations, base_n, nLevels,
      stencil_type);

  if (level < nLevels - 1) {
    // Composite solve on N+1 and above
    RecursiveCompositeSolve( //
        composite_levels, tolerance, max_iterations, base_n, nLevels, level + 1,
        stencil_type);

    // Correction solve on N using N+1/N flux mismatch
    SolvePressureCorrection( //
        composite_levels[level].pressure, composite_levels[level + 1].pressure,
        composite_levels[level].geom, composite_levels[level + 1].geom,
        tolerance, max_iterations, base_n, nLevels, stencil_type);
...
static amrex::Real ExpectedPressure( //
  ...
  const amrex::Real phi2 = Phi(x, y, z, centerPlus, centerPlus, centerPlus, dx);
...
public:
  PressureBndryFunc(int base_n, int nLevels)
      : vBaseN(base_n), vNLevels(nLevels) {}

  void operator()( //
      amrex::Box const &bx, amrex::FArrayBox &data, const int dcomp,
    ...
    amrex::ParallelFor( //
        bx, numcomp, [=] AMREX_GPU_DEVICE(int i, int j, int k, int n) noexcept {
...
amrex::Geometry DefineGeometry( //
  ...
  const amrex::Box domain(                    //
      amrex::IntVect(0, 0, 0), amrex::IntVect(n - 1, n - 1, n - 1));
...
amrex::BoxArray DefineSparseBoxArray(int base_n, int level) {
  ...
  const amrex::Box sparseBox( //
      amrex::IntVect(lo, lo, lo), amrex::IntVect(hi, hi, hi));
...
static amrex::DistributionMapping DefineIOProcessorDM( //
  ...
  return amrex::DistributionMapping(amrex::Vector<int>( //
      ba.size(), amrex::ParallelDescriptor::IOProcessorNumber()));
}
...
void DefineFABs( //
  ...
  for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
    const auto face_ba = amrex::convert( //
        ba, amrex::IntVect::TheDimensionVector(idim));
...
static void InitializeVelocity( //
    std::array<amrex::MultiFab, 3> &velocity, const amrex::Geometry &geom,
    int base_n, int nLevels, int level) {
  if (level == nLevels - 1) {
...
void SamplePressureAlongLine( //
  ...
  amrex::MultiFab full_fine_pressure( //
      fine_ba, fine_dm, fullFineSolution.pressure.nComp(),
  ...
  const amrex::Real center_y =
      0.5 * (fine_geom.ProbLo(1) + fine_geom.ProbHi(1));
  const amrex::Real center_z =
      0.5 * (fine_geom.ProbLo(2) + fine_geom.ProbHi(2));
  ...
  if (amrex::ParallelDescriptor::IOProcessor()) {
    ...
    for (int i = level0_domain.smallEnd(0); i <= level0_domain.bigEnd(0); ++i) {
      const amrex::Real x = level0_geom.CellCenter(i, 0);
      amrex::Real val = 0.0;
      bool found = false;
      for (amrex::MFIter mfi(level_data[0].pressure); mfi.isValid(); ++mfi) {
        const amrex::Box &box = mfi.validbox();
      ...
      const amrex::Real expected = ExpectedPressure( //
          level0_geom, i, j0, k0, base_n, nLevels);
  ...
  for (int lev = 0; lev <= finest_lev; ++lev) {
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE( //
        ratio >= 1, "Fine level must be refined version of current level");
    ...
    const amrex::DistributionMapping partial_domain_dm =
        DefineIOProcessorDM(partial_domain_ba);
    partial_domain_interpolated_pressure[lev].define( //
        partial_domain_ba, partial_domain_dm, ncomp, nGrow);
  ...
  for (int lev = 0; lev <= finest_lev; ++lev) {
    ...
    amrex::PhysBCFunct<PressureBndryFunc> crse_physbc(level_data[lev].geom, bcr,
                                                      pbf);
  ...
  if (amrex::ParallelDescriptor::IOProcessor()) {
    ...
    for (int i = fine_domain.smallEnd(0); i <= fine_domain.bigEnd(0); ++i) {
      ...
      for (int lev = 0; lev <= finest_lev; ++lev) {
        ...
        for (amrex::MFIter mfi(full_domain_interpolated_pressure[lev]);
          ...
          if (box.contains(amrex::IntVect(i, j, k))) {
            const auto &parr =
                full_domain_interpolated_pressure[lev].array(mfi);
            val = parr(i, j, k);
            found = true;
            break;
          }
...
void SingleLevelPressureSolve( //
  ...
  amrex::MultiFab divergence( //
      pressure.boxArray(), pressure.DistributionMap(),
  ...
  SolvePressureIterations( //
      use_expected_BCs, pressure, divergence, geom, tolerance, max_iterations,
...
void CompareMultiFabs( //
  ...
  for (int lev = 0; lev < level_data.size(); ++lev) {
    ...
    if (expected_geom.Domain() == level_geom.Domain()) {
      ...
      amrex::IntVect ratio =
          expected_geom.Domain().size() / level_geom.Domain().size();
...
static void GaussSeidelIteration( //
    bool use_expected_BCs, amrex::MultiFab &pressure,
    amrex::MultiFab &pressure_tmp, const amrex::MultiFab &divergence,
    const amrex::Geometry &geom, int iteration, int base_n, int nLevels,
    const PressureSolve::StencilBase &stencil) {
  const amrex::Real dx = geom.CellSize(0);
  ...
  pressure_tmp.ParallelCopy( //
      pressure, 0, 0, 1, pressure.nGrow(), pressure_tmp.nGrow());
...
static void SolvePressureIterations( //
  ...
  while (residual > tolerance && iteration < max_iterations) {
    GaussSeidelIteration( //
        use_expected_BCs, pressure, pressure_tmp, divergence, geom, iteration,
        base_n, nLevels, stencil);
...
LevelData MakeDenseLevelData( //
  ...
  InitializeVelocity( //
      level_data.velocity, level_data.geom, base_n, nLevels, level);
  return level_data;
}
...
void FillPressureGhostCells( //
  ...
  amrex::PhysBCFunct<amrex::CpuBndryFuncFab> cphysbc( //
      crse_level.geom, bcrecs, null_bndry_func);
  amrex::PhysBCFunct<amrex::CpuBndryFuncFab> fphysbc( //
      fine_level.geom, bcrecs, null_bndry_func);
  ...
  amrex::FillPatchTwoLevels( //
      ...
      {time_unused}, scomp, dcomp, ncomp, crse_level.geom, fine_level.geom,
      cphysbc, cbccomp, fphysbc, fbccomp, ref_ratio,
      // Interpolator makes no difference here?
      // &amrex::pc_interp,
      // &amrex::cell_bilinear_interp,
      &amrex::quadratic_interp, bcrecs, bcscomp);
}
...
amrex::MultiFab ComputeFluxCorrection( //
  ...
  amrex::FluxRegister flux_reg( //
      fine_pressure.boxArray(), fine_pressure.DistributionMap(), ratio,
      ignoredFineLevel, 1);
  ...
  for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
    ...
    amrex::BoxArray ba_crse = amrex::convert( //
        crse_pressure.boxArray(), amrex::IntVect::TheDimensionVector(dir));
    amrex::BoxArray ba_fine = amrex::convert( //
        fine_pressure.boxArray(), amrex::IntVect::TheDimensionVector(dir));
  ...
  for (amrex::MFIter mfi(crse_pressure); mfi.isValid(); ++mfi) {
    ...
    for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
      ...
      amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
        ...
        if (dir == 0) {
          flux_arr(i, j, k) =
              scale * (pres_arr(i, j, k) - pres_arr(i - 1, j, k));
        } else if (dir == 1) {
          flux_arr(i, j, k) =
              scale * (pres_arr(i, j, k) - pres_arr(i, j - 1, k));
        } else {
          flux_arr(i, j, k) =
              scale * (pres_arr(i, j, k) - pres_arr(i, j, k - 1));
        }
      });
    }
  }
  ...
  amrex::MultiFab correction_div( //
      crse_pressure.boxArray(), crse_pressure.DistributionMap(),
      1,  // ncomp
...
void SolvePressureCorrection( //
  ...
  const amrex::MultiFab correction_div = ComputeFluxCorrection( //
      crse_pressure, fine_pressure, crse_geom, fine_geom, stencil_type);
  ...
  SolvePressureIterations( //
      use_expected_BCs, correction_solution, correction_div, crse_geom,
      tolerance, max_iterations, base_n, nLevels,
  ...
  amrex::MultiFab::Add(     //
      crse_pressure, correction_solution, srccomp, dstcomp, numcomp, nghost);
...
/* int CalculateFineN( int base_n, int nLevels )
{
  return CalculateNForLevel( base_n, nLevels - 1 );
}
 int CalculateNForLevel( int base_n, int level )
{
