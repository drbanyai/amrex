...
int MyMain() {
  ...
  std::vector<LevelData> composite_levels = //
      MakeSparseCompositeLevels(            //
          baseN, nlevels, domain_length);

  // Solve on composite mesh
  CompositeSolve( //
      composite_levels, tolerance, max_iterations, baseN, nlevels,
      stencil_type);

  for (int lev = 0; lev < nlevels; ++lev) {
    CheckResults( //
        composite_levels[lev].pressure, composite_levels[lev].geom, baseN,
        nlevels);
  }
  ...
  if (dense) {
    ...
    LevelData dense_level = MakeDenseLevelData( //
        domain_length, CalculateFineN(baseN, nlevels),
        1,  // Only 1 level
    ...
    SingleLevelPressureSolve( //
        dense_level.pressure, dense_level.velocity, dense_level.geom, tolerance,
        max_iterations, baseN, nlevels, stencil_type);
