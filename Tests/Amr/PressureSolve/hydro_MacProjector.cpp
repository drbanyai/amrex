...
MacProjector::MacProjector( const Vector<Geometry>& a_geom,
...
void MacProjector::initProjector( LPInfo a_lpinfo,
                                  const Vector<Array<MultiFab const*, AMREX_SPACEDIM>>& a_beta,
                                  const Vector<iMultiFab const*>& a_overset_mask )
{
  ...
  for ( int ilev = 0; ilev < nlevs; ++ilev ) {
    ba[ilev] = amrex::convert( a_beta[ilev][0]->boxArray(), IntVect::TheZeroVector() );
  ...
  m_divu.resize( nlevs );
  m_needs_level_bcs.assign( nlevs, true );  // TODO

  {
    for ( int ilev = 0; ilev < nlevs; ++ilev ) {
      ...
      for ( int idim = 0; idim < AMREX_SPACEDIM; ++idim ) {
        m_fluxes[ilev][idim].define( amrex::convert( ba[ilev],
    ...
    if ( m_use_mlhypre ) {
      a_lpinfo.setMaxCoarseningLevel( 0 );
    }

    fprintf( stderr, "Creating MLABecLaplacian\n" );
...
void MacProjector::updateBeta( const Vector<Array<MultiFab const*, AMREX_SPACEDIM>>& a_beta )
{
  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_linop != nullptr,
                                    "MacProjector::updateBeta: initProjector must be called before "
                                    "calling this method" );

  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_poisson == nullptr,
                                    "MacProjector::updateBeta: should not be called for constant "
                                    "beta" );

  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_has_robin == false,
                                    "MacProjector::updateBeta: should not be called with Robin BC. "
                                    "Call updateCoeffs" );

  updateCoeffs( a_beta );
}

void MacProjector::updateCoeffs( const Vector<Array<MultiFab const*, AMREX_SPACEDIM>>& a_beta )
{
  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_linop != nullptr,
                                    "MacProjector::updateCoeffs: initProjector must be called "
                                    "before calling this method" );

  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_poisson == nullptr,
                                    "MacProjector::updateCoeffs: should not be called for constant "
                                    "beta" );
  ...
  const auto nlevs = int( a_beta.size() );
  {
    ...
    for ( int ilev = 0; ilev < nlevs; ++ilev ) m_abeclap->setBCoeffs( ilev, a_beta[ilev] );
  }
}

void MacProjector::setUMAC( const Vector<Array<MultiFab*, AMREX_SPACEDIM>>& a_umac )
{
  m_umac = a_umac;
}
...
{
  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_linop != nullptr,
                                    "MacProjector::setDivU: initProjector must be called before "
                                    "calling this method" );

  for ( int ilev = 0, N = int( a_divu.size() ); ilev < N; ++ilev ) {
    if ( a_divu[ilev] ) {
      if ( !m_divu[ilev].ok() ) {
        m_divu[ilev].define( a_divu[ilev]->boxArray(), a_divu[ilev]->DistributionMap(), 1, 0 );
      }
...
void MacProjector::setDomainBC( const Array<LinOpBCType, AMREX_SPACEDIM>& lobc,
                                const Array<LinOpBCType, AMREX_SPACEDIM>& hibc )
{
  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_linop != nullptr,
                                    "MacProjector::setDomainBC: initProjector must be called "
                                    "before calling this method" );
...
void MacProjector::setLevelBC( int amrlev,
                               const MultiFab* levelbcdata,
                               const MultiFab* robin_a,
                               const MultiFab* robin_b,
                               const MultiFab* robin_f )
{
...
void MacProjector::project( Real reltol, Real atol )
{
...
void MacProjector::project( const Vector<MultiFab*>& phi_inout, Real reltol, Real atol )
{
  const auto nlevs = int( m_rhs.size() );
  for ( int ilev = 0; ilev < nlevs; ++ilev ) {
...
void MacProjector::project_doit( Real reltol, Real atol )
{
  const auto nlevs = int( m_rhs.size() );
  ...
  assert( m_umac[0][0] );
  if ( m_umac[0][0] ) {
    averageDownVelocity();  // Skip the average down...why?
  }
  ...
  for ( int ilev = 0; ilev < nlevs; ++ilev ) {
    ...
    if ( m_umac[0][0] ) {
      ...
      // printf( "m_geom[ilev].InvCellSizeArray() = %g, %g, %g\n", dxinv[0], dxinv[1], dxinv[2] );

      computeDivergence( m_rhs[ilev], u, m_geom[ilev] );
      // printf( "m_rhs[%d] Divergence:\n", ilev );
      // for ( amrex::MFIter mfi( m_rhs.at( ilev ) ); mfi.isValid(); ++mfi ) {
      //   const auto rhs_array = m_rhs.at( ilev ).const_array( mfi );
      ...
      assert( m_poisson );
      // printf( "1.0 / m_const_beta = %g\n", 1.0 / m_const_beta );
      assert( m_poisson ? Real( 1.0 ) / m_const_beta : Real( -1.0 ) == 1.0 );
      // no-op, multiplies by +1.0.
      m_rhs[ilev].mult( m_poisson ? Real( 1.0 ) / m_const_beta : Real( -1.0 ) );
    }
    // else m_rhs already initialized to 0
    if ( m_divu[ilev].ok() ) {
      assert( false );
      MultiFab::Saxpy( m_rhs[ilev],
                       m_poisson ? Real( -1.0 ) / m_const_beta : Real( 1.0 ),
                       m_divu[ilev],
                       0,
                       0,
                       1,
  ...
  if ( m_umac[0][0] ) {
    ...
    averageDownVelocity();  // Necessary
  }
}

void MacProjector::getFluxes( const Vector<Array<MultiFab*, AMREX_SPACEDIM>>& a_flux,
                              const Vector<MultiFab*>& a_sol,
                              MLMG::Location a_loc ) const
{
  int ilev = 0;
  if ( m_needs_level_bcs[ilev] ) {
    m_linop->setLevelBC( ilev, nullptr );
...
void MacProjector::averageDownVelocity()
{
...
#if defined( AMREX_USE_EB ) && !defined( HYDRO_NO_EB )
    EB_average_down_faces( GetArrOfConstPtrs( m_umac[lev] ), m_umac[lev - 1], rr, m_geom[lev - 1] );
#else
    average_down_faces( GetArrOfConstPtrs( m_umac[lev] ), m_umac[lev - 1], rr, m_geom[lev - 1] );
#endif
  }
}
                                  ...
                                  const Vector<iMultiFab const*>& a_overset_mask )
{
  ...
  fprintf( stderr, "Creating poisson\n" );
  if ( a_overset_mask.empty() ) {
...
#  ifdef AMREX_USE_HYPRE
  if ( m_use_mlhypre ) {
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE( a_overset_mask.empty(),
                                      "mlhypre does not support overset mask yet" );
  }
#  endif
...
{
  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_linop != nullptr,
                                    "MacProjector::updateBeta: initProjector must be called "
                                    "before "
                                    "calling this method" );

  AMREX_ALWAYS_ASSERT_WITH_MESSAGE( m_poisson != nullptr,
                                    "MacProjector::updateBeta: should not be called for variable "
                                    "beta" );
